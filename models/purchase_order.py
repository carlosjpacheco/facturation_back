    ...
    try:
        ...
        if valid == True:
            ...
            if request["preview"] == False:
                ...
                records = (request["id_user"],datetime.strptime(request["date"],"%d/%m/%Y").timestamp(),False,False,request["supplier"],request["terms_conditions"],request["delivery_address"],request["currency"],'',)
...
async def addPurchaseOrderDetail(request):
    ...
    cursor["cursor"].execute(sql_update,(path,request["supplier"],))
