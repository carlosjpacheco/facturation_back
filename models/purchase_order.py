    ...
    try:
        ...
        if valid == True:
            ...
            if request["preview"] == False:
                print(request["id_user"],datetime.strptime(request["date"],"%Y-%m-%dT%H:%M:%S.%fZ").timestamp(),False,False,request["supplier"],request["terms_conditions"],request["delivery_address"],request["currency"],'',)
